 ...
 * Time: 6:15 AM
 */
class OrderGroupingFixLineHandlerConsoleTest extends Specification {
...
"""
    ...
    def "another single order, with orphan messages"() {
        given:
        final StringBuilder output = new StringBuilder()
        final ConfigKeyedWithOption config = new ConfigBuilder(['--output-format-horizontal-console', '\${senderToTargetCompIdDirection} [\${msgTypeName}] \${msgFix}', '--suppress-bold-tags-and-values', 'true', '-e', '49,56,150', '-a', 'rr']).configAndArguments.config
        final FormatSpec formatSpec = new FormatSpec(config)
