...
class ExampleAppPropertiesFileCreator {
    ...
    fun createIfNecessary() {
        if(FIXGREP_APP_PROPERTIES_FILE.exists()){
            logger.info("Fixgrep properties file already exists at: " + FIXGREP_APP_PROPERTIES_FILE.absolutePath)
            return
        }

        logger.info("Attempting to create application.properties file at: " + FIXGREP_APP_PROPERTIES_FILE.absolutePath + "\nPlease modify config in this file to change the behaviour of fixgrep for every time it is run.")
        try {
            ...
            logger.error("Could not create application.properties  reason:[" + e.message + "]")
            return
        }
    }
}