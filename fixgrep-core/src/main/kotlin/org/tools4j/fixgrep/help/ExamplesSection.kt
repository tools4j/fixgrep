...
class ExamplesSection(val docWriterFactory: DocWriterFactory) {
    fun toFormattedText(): String {
        ...
        examplesList.add("--tag-annotations outsideAnnotated", "Set annotations to 'outside' meaning the annotated tag will sit before the tagRaw, and the annotated value will sit after the valueRaw.  Like this [tagAnnotation]tagRaw=valueRaw[valueAnnotation], e.g. [Side]54=2[SELL].  This is the default annotation format used by fixgrep, so don't be surprised if the formatted fix did not change.")
        ...
        examplesList.add("-a insideAnnotated", "Inside annotation, meaning the annotated tag will sit after the tagRaw, and the annotated value will sit before the valueRaw.  Like this tagRaw[tagAnnotation]=[valueAnnotation]valueRaw, e.g. 54[Side]=[SELL]2")
