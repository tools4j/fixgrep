...
class ManGenerator(val docWriterFactory: DocWriterFactory, val config: FixGrepConfig, val debug: Boolean = false) {
    ...
    private fun howToUseOutputFormats(): String {
        ...
        with(writer) {
            ...
            writeHeading(2, Option.output_format_horizontal_console.key)
            writeCode(config.outputFormatHorizontalConsole + "\n")
            writeHeading(2, Option.output_format_horizontal_html.key)
            writeCode(config.outputFormatHorizontalHtml + "\n")
            writeHeading(2, Option.output_format_vertical_console.key)
            writeCode(config.outputFormatVerticalConsole + "\n")
            writeHeading(2, Option.output_format_vertical_html.key)
            writeCode(config.outputFormatVerticalHtml + "\n")
            return toFormattedText()
        }
    }
