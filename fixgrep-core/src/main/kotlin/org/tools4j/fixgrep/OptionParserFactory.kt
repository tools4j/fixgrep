...
class OptionParserFactory{
    ...
    val optionsThatShouldNotHaveEquivalentProperties: List<String> by lazy {
        listOf("?", "help", "[arguments]", "man", "256-color-demo", "16-color-demo", "gimme-css", "html", "launch-browser")
    }
    ...
    val optionParser: OptionParser by lazy {
        val parser = object: OptionParser() {
            init {
                ...
                acceptsAll(asList("F", "line-format")).withRequiredArg().ofType(String::class.java) // "$1 ${senderToTargetCompIdDirection} ${msgColor}[${msgTypeName}]${colorReset} ${msgFixOutsideAnnotated}"
