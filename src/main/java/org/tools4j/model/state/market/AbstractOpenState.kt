...
abstract class AbstractOpenState(val context: StateContext) : AbstractState(context) {
    ...
    override fun handleCancelReplaceRequest(message: CancelReplaceRequest) {
        if(message.orderQty < ( context.order.qty - context.order.leavesQty) ){
            val replaceRejected = message.createRejectedMessage(context.execIDGenerator.get(), context.order.ordStatus, context.order.leavesQty, context.order.cumQty, "Requested amend qty ${message.orderQty} is less the cumQty ${context.order.cumQty}" )
            ...
            val replaceAccepted = message.createAcceptedMessage(context.execIDGenerator.get(), context.order.ordStatus, context.order.leavesQty, context.order.cumQty)
